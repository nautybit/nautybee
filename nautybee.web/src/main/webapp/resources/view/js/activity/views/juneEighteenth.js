...
function init(){
    ...
    gViewModel.groupLength = {
        LYG:subActivityList[0].activityGoodsViewList.length,
        BJ:subActivityList[1].activityGoodsViewList.length
    };
...
function getDevice(){
    ...
    if( (/iphone|ipod|ipad/gi).test(ua) ){
        return 'ios';
    }else if ((/android/gi).test(ua) && (/mobile/gi).test(ua)){
        return 'android';
    }else{
        return 'unknown';
    }
...
function mainMenuClick(key){
    ...
    if($('#dataStorage>div[groupKey='+key+']').length>0){
        ...
        $('#goodsBottom').before($('#dataStorage>div[groupKey='+key+']').clone().children('.goodsBlock'));
        menuChanged();
        $("#wrapper .img-goods").lazyload();
    }else{
        var goodsLength;
        if(key == "LYG"){
            goodsLength = gViewModel.groupLength.LYG;
        }else{
            goodsLength = gViewModel.groupLength.BJ;
        }
        if(goodsLength == 0){
            $('#wrapper .goodsBlock').remove();
            $('#noGoodsInfo').css("display","block");
            menuChanged();
            return;
        }
        fetchRender(key);
...
function fetchRender(key){
    ...
    var goodsList;
    if(key == "LYG"){
        goodsList = subActivityList[0].activityGoodsViewList;
    }else{
        goodsList = subActivityList[1].activityGoodsViewList;
    }
    $('#noGoodsInfo').css("display","none");
    ...
    for(var i = 0;i<goodsList.length;i++){
        ...
        if(groupKey=="LYG"){
            $(obj).find('.groupIco').append('<img width="100%" src='+free_ico+'>');
        }else{
            $(obj).find('.groupIco').append('<img width="100%" src='+half_ico+'>');
        }
        if(soldOut == "Y"){
    ...
    $('#wrapper .goodsBlock').remove();
    $('#goodsBottom').before($container.clone().children('.goodsBlock'));
...
function windowResized(){
    ...
    $('#bought').css({
        'width':550/720*getScreenWidth()+'px',
        'height':344/720*getScreenWidth()+'px',
        'position':'absolute',
        'top':450/720*getScreenWidth()+'px',
        'right':85/720*getScreenWidth()+'px'
    });
    $('#noGoodsInfo').css({
        'position':'absolute',
        'bottom':'90px',
    ...
    $('#sad').css({
        ...
        'height':190/720*getScreenWidth()+'px'
    });
    $('#sorry').css({
        'margin':36/720*getScreenWidth()+'px',
        'color':'#212121',
        'font-size':36/720*getScreenWidth()+'px'
    });
    $('.reason').css({
        'color':'#666666',
        'font-size':28/720*getScreenWidth()+'px'
    });
    $('.outStockPic').css({
...
function refreshScroll(){
    setTimeout(function(){
        ...
        $('#bigCover').css({
            ...
            'height':document.getElementById('wrapper').scrollHeight+'px',
            'top':0,
            'left':0
        });
        if(gViewModel.$scrollMenuBar.get(0).offsetTop+gPageScroll.y < 0){
            gViewModel.$fixedMenuBar.css("display","block");
        }else{
...
function adjustMenuDisplay(){
    ...
    if($scrollMenuBar.get(0).offsetTop+gPageScroll.y <0 && $('#bigCover').css("display")=="none"){
